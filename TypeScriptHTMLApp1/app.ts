module App {
    ...
    app.controller("BaseController", ["$scope", "applicationInsightsService", ($scope, applicationInsightsService) => new Controllers.BaseController($scope, applicationInsightsService)]); 
    app.controller("StaticController", ["$scope", "applicationInsightsService", ($scope, applicationInsightsService) => new Controllers.StaticController($scope, applicationInsightsService)]);
    app.controller("HomePageController", ["$scope", "$state", "applicationInsightsService", ($scope, $state, applicationInsightsService) => new Controllers.HomePageController($scope, $state, applicationInsightsService)]);
    app.controller("ItemsController", ["$scope", "$state", "ItemService", "$uibModal", "growl", "applicationInsightsService", ($scope, $state, ItemService, $uibModal, growl, applicationInsightsService) => new Controllers.ItemsController($scope, $state, ItemService, $uibModal, growl, applicationInsightsService)]);
    app.controller("ShoppingCartController", ["$scope", "$state", "ItemService", "growl", "applicationInsightsService", ($scope, $state, ItemService, growl, applicationInsightsService) => new Controllers.ShoppingCartController($scope, $state, ItemService, growl, applicationInsightsService)]);
    app.controller("CheckoutController", ["$scope", "$state", "ItemService", "growl", "applicationInsightsService", ($scope, $state, ItemService, growl, applicationInsightsService) => new Controllers.CheckoutController($scope, $state, ItemService, growl, applicationInsightsService)]);
   

    // Services
    app.factory("HttpHandler", ["$http", ($http) => new Services.Base.HttpHandler($http)]);
    app.factory("ItemService", ["HttpHandler", (HttpHandler) => new Services.Items.ItemService(HttpHandler)]);
